/*
 * Author: SeaYJ
 * Desc: 源代码在 Github: https://github.com/SeaYJ/LanternJS
 */
class Lantern {
    #startDate = '1-1';
    #endDate = '1-31';
    #dateCheck(a) {
        let b = a['split']('-')[0x0],
            c = a['split']('-')[0x1];
        if (b < 0x1 || b > 0xc) return console['error']('ERROR:\x20%s\x20is\x20a\x20problem!The\x20month\x20can\x20only\x20be\x20selected\x20from\x201~12!', b), ![];
        if (c < 0x1 || c > 0x1f) return console['error']('ERROR:\x20%s\x20is\x20a\x20problem!Dates\x20can\x20only\x20be\x20selected\x20from\x201~31!You\x20don\x27t\x20need\x20to\x20think\x20about\x20leap\x20years,\x20you\x20just\x20need\x20to\x20treat\x20it\x20as\x20if\x20every\x20month\x20is\x2031\x20days,\x20and\x20we\x27ll\x20do\x20the\x20rest!', c), ![];
        return !![];
    }
    #dateCorrect(a) {
        !('date' in a) ? a['date'] = {
            'from': this.#startDate,
            'to': this.#endDate
        }: ((!('from' in a['date']) || !this.#dateCheck(a['date']['from'])) && (a['date']['from'] = this.#startDate), (!('to' in a['date']) || !this.#dateCheck(a['date']['to'])) && (a['date']['to'] = this.#endDate));
    }
    #CompareTime(a, b) {
        let c = a['split']('-')[0x0],
            d = a['split']('-')[0x1],
    ...
    #postionCorrect(a) {
        if ('postion' in a) {
            'zIndex' in a['postion'] && (this.#HTMLContent['zIndex'] = a['postion']['zIndex']);
    ...
    constructor(a, b) {
        ...
        this.#CompareTime(b['date']['from'], b['date']['to']) ? c['innerHTML'] = this.#HTMLContent['HTML']() : c['innerHTML'] = '';
    }
}